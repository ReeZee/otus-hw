...
fn main() {
    let (res, execution_time) = measure_time(|(a, b)| {
        sum(a,b)
    }, (10, 20));
    assert_eq!(30, res);
    println!("sum took {execution_time:?}");

    let (res, execution_time) = measure_time(|strs| {
        concat(strs)
    }, &["hello", "world"]);
    assert_eq!("hello world", res);
    println!("concat took {execution_time:?}");

    let (_, execution_time) = measure_time(|_| {
...
pub fn measure_time<T, R>(f: fn(T) -> R, arg: T) -> (R, u128) {
    let now = Instant::now();
    (f(arg), now.elapsed().as_nanos())
}
